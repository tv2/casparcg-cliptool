    ...
    switch (action.type) {
        ...
        case 'SET_TIMELEFT':
            action.data.timeLeft.map((item, index) => {
                ...
                nextState[0].ccgInfo[index].layers[9].foreground.paused =
                    state[0].ccgTime[index] === item.time &&
                    state[0].ccgPrevTime[index] === item.time
                ...
                nextState[0].ccgTimeCounter[index] = secondsToTimeCode(
                    item.timeLeft
                )
            })
            return nextState
        case 'SET_MEDIA_PAUSED':
            nextState[0].ccgInfo[action.index].layers[9].foreground.paused =
                action.paused
            return nextState
        case 'SET_THUMB_LENGTH':
            if (
                ...
                if (
                    nextState[0].channel[action.data.tab].thumbActiveIndex >
                ...
                if (
                    nextState[0].channel[action.data.tab].thumbActiveBgIndex >
                    action.data.length - 1
                ) {
                    nextState[0].channel[action.data.tab].thumbActiveBgIndex =
                        action.data.length - 1
        ...
        case 'SET_THUMB_LIST':
            if (
                action.data.index <=
                nextState[0].channel[action.data.tab].thumbList.length - 1
            ) {
                nextState[0].channel[action.data.tab].thumbList[
                    action.data.index
                ] = action.data.thumbList
            } else {
                nextState[0].channel[action.data.tab].thumbList.push(
                    action.data.thumbList
                )
            }
            nextState[0].channel[action.data.tab].thumbList[
        ...
        case 'SET_THUMB_PIX':
            nextState[0].channel[action.data.tab].thumbList[
                action.data.index
            ].thumbPix = action.data.thumbPix
            return nextState
        case 'SET_META_LIST':
            action.metaList.map(
                (metaElement) => (metaElement.elementActive = 0)
            )
        ...
        case 'SET_META_ELEMENT_ACTIVE': // tab, index (thumblist index), elementIndex, active
            nextState[0].channel[action.tab].thumbList[action.index].metaList[
                action.elementIndex
            ].elementActive = action.active
            return nextState
        case 'SET_EMPTY_META':
            nextState[0].channel[action.tab].thumbList[
                action.index
            ].metaList = []
            return nextState
        case 'SET_OVERLAY_IS_STARTED': // tab, layer, started
            nextState[0].channel[action.tab].overlayIsStarted[
                action.layer
        ...
        case 'SET_THUMB_ACTIVE_BG_INDEX':
            ...
            //Set new Bg Tally:
            nextState[0].channel[action.data.tab].thumbList[
                action.data.thumbActiveBgIndex
            ].tallyBg = true
            nextState[0].channel[action.data.tab].thumbActiveBgIndex =
                action.data.thumbActiveBgIndex
            return nextState
